...
sudo su postgres -c psql <<EOF
 SELECT pglogical.create_node(node_name := 'subscriber1', dsn := 'host=192.168.33.10 port=5432 dbname=test_replication');
EOF
...
sudo su postgres -c psql <<EOF
 SELECT pglogical.create_subscription(
     ...
     provider_dsn := 'host=master port=5432 dbname=test_replication'
 );
EOF
